# TH99 Pro protocol — verified notes

Protocol facts for the Epomaker TH99 Pro (`0C45:800A`, wired), **verified against live
traffic** captured from Epomaker's own web driver (`hub.epomaker.com`) on 2026-08-18, and
confirmed by driving the display from catbus99 on macOS.

Prior reverse-engineering credit: [regenbildr/epomaker-th99-pro-AI-usage][up], from USB
captures of the official driver on Windows. This is an independent implementation written
from protocol facts; no upstream source was copied. Where our live capture disagrees with
upstream's notes, the disagreement is called out below.
...
**Correction to upstream:** upstream documents `AA 34` with declared length `0x38` (56).
The live driver sends **`0x18` (24)**. The keyboard echoes whatever it is sent, so the
acknowledgement does *not* validate this field — a wrong value passes unnoticed.
...
the `AA 50` report stream — verified: a `Write Image` capture contains exactly 16 sends,
all `AA 50`, and nothing else on either interface.
...
The table is the firmware's **key matrix — 16 columns by 7 rows, 112 entries** — not the
...
**Correction to upstream:** upstream states there is no way to clear the screen. There is
— the driver's `Clear Screen` is simply a **single-frame all-black image upload**
...
single-frame container is also valid and displays fine — `Clear Screen` proves it — and
